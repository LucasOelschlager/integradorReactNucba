...
export const Categorys = () => {
    const { categorySet, setCategorySet, setCurrentPage } = useContext(CategoryContext)
    return (
        <div className={`${styledCategorys.categoryContainer}`}>
            ...
            <div className={`${styledCategorys.container}`}>
                <ul className={`${styledCategorys.list}`}>
                    {categorias.map((e) => (
                        <li key={e.id} className={categorySet == e.name.toLowerCase() ? `${styledCategorys.categoryItemAct}` : styledCategorys.categoryItem} onClick={() => {
